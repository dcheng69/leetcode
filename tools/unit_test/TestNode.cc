#include <gtest/gtest.h>
#include <gmock/gmock.h>
#include "Node.h"
#include "PrintX.h"
using namespace std;
using testing::ElementsAreArray;

TEST(TestNode, TestConstructVector) {
    // test nullptr input
    Node* p_root = nullptr;
    ASSERT_EQ(ConstructVector(p_root).size(), 0);

    // construct a one node Node
    p_root = new Node(0);
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0"}));

    // construct a two node Node with left child
    Node* p_left_child = new Node(1);
    p_root->left = p_left_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "null"}));

    // construct a two node Node with right child
    Node* p_right_child = new Node(2);
    p_root->left = nullptr; // remove the left child
    p_root->right = p_right_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "null", "2"}));

    // construct a with 3 node Node
    p_root->left = p_left_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2"}));

    // construct a with 4 node
    Node* p_left_left_child= new Node(3);
    p_left_child->left = p_left_left_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2", "3", "null", "null", "null"}));

    // construct a with 4 node
    p_left_child->left = nullptr;
    Node* p_left_right_child= new Node(4);
    p_left_child->right = p_left_right_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2", "null", "4", "null", "null"}));

    // construct a with 5 node
    p_left_child->left = p_left_left_child;
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2", "3", "4", "null", "null"}));

    // cleanning up, take care of mem leak!
    free(p_left_right_child);
    p_left_right_child = nullptr;
    free(p_left_left_child);
    p_left_left_child = nullptr;
    free(p_right_child);
    p_right_child = nullptr;
    free(p_left_child);
    p_left_child = nullptr;
    free(p_root);
    p_root = nullptr;
}

TEST(TestNode, TestConstructNode) {
    Node* p_root = nullptr;
    vector<string> s_vec;
    // test null input
    p_root = ConstructNode(s_vec, "null");
    ASSERT_EQ(p_root, nullptr);

    // construct  with one node
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "null", "null"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0"}));

    // construct two node  with left child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "1", "null"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "null"}));

    // construct two node  with right child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "null", "1"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0","null", "1"}));

    // construct three node  with left and right child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "1", "2"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2"}));

    // construct three node  with left and right child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "null", "1", "null", "null", "2"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "null", "1", "null", "null", "2", "null"}));

}


TEST(TestNode, TestConstructFourNode) {
    Node* p_root = nullptr;
    vector<string> s_vec;
    // test null input
    p_root = ConstructNode(s_vec, "null");
    ASSERT_EQ(p_root, nullptr);

    // construct four node  with last layer has only left child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "1", "2", "3", "null", "null", "null"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray({"0", "1", "2", "3", "null", "null", "null"}));


    // construct four node  with last layer has only left child
    p_root = nullptr;
    s_vec.clear();  // content cleared, capacity remains
    s_vec.assign({"0", "4", "1", "null", "null", "2", "null", "null", "null", "null", "null", "null", "3"});
    p_root = ConstructNode(s_vec, "null");
    PrintNode(p_root); cout << endl;
    ASSERT_THAT(ConstructVector(p_root), ElementsAreArray( { "0", "4", "1", "null", "null", "2", "null", "null", "null", "null", "null",
 "null", "3", "null", "null" }));
}

TEST(TestNode, TestSmartNode) {
    vector<string> s_vec{"0", "1", "2", "3", "null", "null", "null"};
    Node* p_root = ConstructNode(s_vec, "null");
    SmartNode s_node(p_root);
    ASSERT_EQ(s_node.GetRootNodePointer(), p_root);

    Node* p_root_need_free = s_node.RebindRootPointer(nullptr);
    ASSERT_EQ(s_node.GetRootNodePointer(), nullptr);

    SmartNode::FreeNode(p_root_need_free);
    p_root_need_free = nullptr;
}

TEST(TestNode, TestMaxDepth) {
    // test nullptr input
    Node* p_root = nullptr;
    ASSERT_THAT(MaxNodeDepth(p_root), 0);

    // construct a one node Node
    p_root = new Node(0);
    ASSERT_THAT(MaxNodeDepth(p_root), 1);
    SmartNode* st = new SmartNode(p_root);

    // construct a two node Node with left child
    Node* p_left_child = new Node(1);
    p_root->left = p_left_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 2);

    // construct a two node Node with right child
    Node* p_right_child = new Node(2);
    p_root->left = nullptr; // remove the left child
    p_root->right = p_right_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 2);

    // construct a with 3 node Node
    p_root->left = p_left_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 2);

    // construct a with 4 node
    Node* p_left_left_child= new Node(3);
    p_left_child->left = p_left_left_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 3);

    // construct a with 4 node
    p_left_child->left = nullptr;
    Node* p_left_right_child= new Node(4);
    p_left_child->right = p_left_right_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 3);

    // construct a with 5 node
    p_left_child->left = p_left_left_child;
    ASSERT_THAT(MaxNodeDepth(p_root), 3);

    delete st;
    p_root = nullptr;
}

TEST(TestNode, TestMaxNodeValue) {
    // test nullptr input
    Node* p_root = nullptr;
    ASSERT_THAT(MaxNodeValue(p_root), 0);

    // construct a one node Node
    p_root = new Node(0);
    ASSERT_THAT(MaxNodeValue(p_root), 0);
    SmartNode* st = new SmartNode(p_root);

    // construct a two node Node with left child
    Node* p_left_child = new Node(1);
    p_root->left = p_left_child;
    ASSERT_THAT(MaxNodeValue(p_root), 1);

    // construct a two node Node with right child
    Node* p_right_child = new Node(2);
    p_root->left = nullptr; // remove the left child
    p_root->right = p_right_child;
    ASSERT_THAT(MaxNodeValue(p_root), 2);

    // construct a with 3 node Node
    p_root->left = p_left_child;
    ASSERT_THAT(MaxNodeValue(p_root), 2);

    // construct a with 4 node
    Node* p_left_left_child= new Node(3);
    p_left_child->left = p_left_left_child;
    ASSERT_THAT(MaxNodeValue(p_root), 3);

    // construct a with 4 node
    p_left_child->left = nullptr;
    Node* p_left_right_child= new Node(4);
    p_left_child->right = p_left_right_child;
    ASSERT_THAT(MaxNodeValue(p_root), 4);

    // construct a with 5 node
    p_left_child->left = p_left_left_child;
    ASSERT_THAT(MaxNodeValue(p_root), 4);

    // cleanning up, take care of mem leak!
    delete st;
    p_root = nullptr;
}
